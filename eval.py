        ...
        for name in corrects:
            ...
            if "top-1" in name or "highest" in name:
                if acc >= best_top1:
                    best_top1 = acc
                    best_top1_name = name
        evaluate_cm(args, model, test_loader)
...
#     plt.figure(figsize=(len(label_names) / 1.2, len(label_names) / 1.2), dpi=200)
...
#             c = 0
#         if c > 0.001:
#             plt.text(x_val, y_val, "%0.1f" % (c,), color='red', fontsize=10, va='center', ha='center')
...
def plot_confusion_matrix(cm, label_names, save_name, title='Confusion Matrix acc = ', accuracy=0):
    ...
    for i in range(len(label_names)):
        for j in range(len(label_names)):
            plt.text(j, i, str(cm[i, j]), va='center', ha='center', color='red', fontsize=10)
