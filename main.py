...
from eval import evaluate, cal_train_metrics, evaluate_cm
...
def cross_validate(args, num_folds=10, random_seed=None):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(range(len(train_loader.dataset)))):
        ...
        for epoch in range(start_epoch, args.max_epochs):
            # Train
            if train_loader_fold is not None:
                tlogger.print(f"Start Training Epoch {epoch + 1}")
                train(args, epoch, model, scaler, amp_context, optimizer, schedule, train_loader_fold)
                tlogger.print()

        accuracy, eval_name, accs = evaluate(args, model, val_loader_fold)
    ...
    if not np.isnan(mean_accuracy) and not np.isnan(std_deviation):
        print(f"Mean accuracy: {mean_accuracy}%")
        print(f"Standard deviation: {std_deviation}")
    else:
        print("An error occurred during cross-validation. Please check your data and settings.")
...
def set_environment(args, tlogger):
    ...
    if args.pretrained is not None:
        checkpoint = torch.load(args.pretrained, map_location=torch.device('cuda:0'))
        ...
        # ignoring layers that don't match in size.
    ...
    if args.optimizer == "SGD":
        ...
        optimizer = Lion(model.parameters(), lr=args.max_lr, weight_decay=args.wdecay)
    elif args.optimizer == "SWATS":
        optimizer = swats.SWATS(model.parameters(), lr=args.max_lr, nesterov=True, weight_decay=args.wdecay)
...
def main(args, tlogger):
    ...
    # cross_validate(args, random_seed=42)
    ...
    for epoch in range(start_epoch, args.max_epochs):
        ...
        if train_loader is not None:
            ...
            eval_and_save(args, model, val_loader, tlogger=tlogger)
            break
